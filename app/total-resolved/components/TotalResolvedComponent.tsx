...
const TotalResolvedComponent = () => {
  ...
  const role = getRole();
  const ALLOWED_STATUS = [4, 5, 6, 7];

  const { data: complaintData } = useGetAllComplains({
    refresh,
  ...
  let filteredData =
    complaintData?.filter((c) => ALLOWED_STATUS.includes(c.status)) ?? [];

  if (search && filteredData.length) {
...
export default TotalResolvedComponent;
