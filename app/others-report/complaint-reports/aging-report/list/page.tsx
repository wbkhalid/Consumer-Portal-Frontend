...
const AgingReportPage = async ({ searchParams }: Props) => {
  ...
  let data: AgingReport[] = response.data;
  ...
  if (search) {
    ...
    data = data.filter(
      (d) =>
        d.dayRange.toLowerCase().includes(lowerSearch) ||
        d.numberOfComplaints.toString().includes(lowerSearch)
    );
  }
