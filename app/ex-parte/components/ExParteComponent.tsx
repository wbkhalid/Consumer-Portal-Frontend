...
import ExParteTable from "./ExPartyTable";

const ExParteComponent = () => {
  const [refresh, setRefresh] = useState(false);
  const { divisionId, districtId, tehsilId } = useRegionFilters();
  const { data: expateData } = useGetAllComplains({
    status: 5,
    refresh,
    ...
    <div className="border border-[#e2e8f0] rounded-lg py-1! overflow-hidden max-h-[calc(100vh-10px)]">
      <div className="flex justify-between items-center px-4! py-2!">
        <div className="flex items-center gap-1">
          <p className="text-[#181D27] font-semibold">Ex-Parte</p>
          <p className="border border-main text-main font-semibold rounded-full px-1! py-0.5! text-xs">
            {expateData?.length?.toLocaleString()} Records
          </p>
        </div>
      </div>
      <ExParteTable rowsData={expateData ?? []} setRefresh={setRefresh} />
    </div>
  );
};

export default ExParteComponent;
